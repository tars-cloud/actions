...
function policy(config, context) {
  ...
  const fork = Boolean(
    context.headRepository && context.headRepository.toLowerCase() !== context.repository.toLowerCase(),
  );
...
function discover(root, patterns) {
  ...
  function visit(directory, relative = "") {
    for (const entry of fs
      ...
      if (patterns.some((pattern) => path.matchesGlob(name, pattern) || path.matchesGlob(entry.name, pattern)))
        continue;
...
function cachePlan(config, context, env = process.env, now = new Date()) {
  ...
  for (const file of required) {
    ...
    if (!fs.existsSync(full) || !fs.lstatSync(full).isFile())
      throw new Error(`Selected environment requires ${file} in working-directory (regular files only).`);
  }
  ...
  const fingerprint = (names) =>
    digest(
      [...new Set(names)]
        .sort()
        .map((name) => `${name}\0${read(name).toString("base64")}`)
        .join("\0"),
    );
  const reasons = [];
  ...
  if (
    requested.some((tool) => !["cargo", "python", "bun", "trivy", "none"].includes(tool)) ||
    (requested.includes("none") && requested.length !== 1)
  ) {
  ...
  const bun =
    has(/^bun\.lockb?$/) ||
    files
      .filter((name) => base(name) === "package.json")
      .some((name) => {
        try {
          return /^bun@/.test(JSON.parse(read(name)).packageManager || "");
        } catch {
          reasons.push(`Cannot parse ${name}; use tools: bun if appropriate.`);
          return false;
        }
      });
  ...
  if (automatic && !bun && has(/^package\.json$/))
    reasons.push("package.json alone does not identify Bun; use tools: bun.");
  ...
  function directory(input, variable, fallback) {
    ...
    if (/[\r\n\0*?!\[\]{}]/.test(value))
      throw new Error(`${input} must be one literal directory, without glob patterns or line breaks.`);
    const resolved = path.resolve(root, value);
    if (resolved === "/" || resolved === home || resolved === root)
      throw new Error(`${input} must be a dedicated cache directory.`);
    return resolved;
  }
  const xdg = env.XDG_CACHE_HOME || path.join(home, ".cache");
  const cargoHome = selected.includes("cargo")
    ? directory("cargo-cache-path", "CARGO_HOME", path.join(home, ".cargo"))
    : "";
  const locations = {
    ...
    bun: [
      ...
      path.join(env.BUN_INSTALL || path.join(home, ".bun"), "install/cache"),
    ],
  ...
  for (const tool of selected) {
    ...
    reasons.push(
      `${tool}: ${automatic ? "detected project metadata" : "explicit tool selection"}; ${selection.backend} storage.`,
    );
  }
  const exports = {};
  for (const tool of selected) {
