...
const s3 = { 's3-endpoint': 'https://cache.example.invalid', 's3-bucket': 'fixture', 's3-region': 'us-east-1', 's3-access-key': 'fixture-key', 's3-secret-key': 'fixture-secret' };
const context = { runner: 'self-hosted', os: 'Linux', arch: 'X64', repository: 'example/project', defaultBranch: 'trunk', ref: 'refs/heads/topic' };
function fixture(t, files = {}) {
  ...
  for (const [name, content] of Object.entries({ 'devenv.nix': '{}', 'devenv.yaml': 'inputs: {}', 'devenv.lock': '{}', ...files })) write(name, content);
  return {
    root, write,
    plan: (config = {}, changes = {}, env = {}) => cachePlan(config, { ...context, workspace: root, ...changes }, { HOME: path.join(root, 'home'), ...env }, new Date('2026-09-22T12:00:00Z')),
  };
}
...
test('backend routing covers hosted, unconfigured self-hosted, complete and partial S3', () => {
  ...
  assert.throws(() => policy({ 's3-bucket': 'fixture' }, context), /s3-endpoint, s3-region, s3-access-key, s3-secret-key/);
  assert.throws(() => policy({ 's3-session-token': 'secret-value' }, context), error => !error.message.includes('secret-value'));
});
...
test('fork policy precedes S3 validation and disables Cachix writes for any PR-bearing event', () => {
  for (const runner of ['self-hosted', 'github-hosted']) {
    const result = policy({ 's3-bucket': 'unused', 'cachix-name': 'public', 'cachix-token': 'secret' }, { ...context, runner, headRepository: 'fork/project', pr: 7 });
    assert.equal(result.backend, 'github');
    assert.equal(result.cachix, 'read');
    assert.equal(result.fork, true);
  }
  assert.equal(policy({ ...s3, 'cachix-name': 'public', 'cachix-token': 'secret' }, { ...context, headRepository: context.repository, pr: 7 }).cachix, 'write');
...
test('nested mixed detection excludes installations, scratch, workflow Trivy names and symlinks', t => {
  const f = fixture(t, {
    'rust/Cargo.toml': '[package]', 'python/uv.lock': 'uv', 'python/pyproject.toml': '[project]',
    'legacy/requirements.txt': 'example==1', 'frontend/bun.lockb': 'binary', 'security/trivy.yaml': '{}',
    'scratch/ignored/bun.lock': 'ignored', 'node_modules/fake/Cargo.toml': 'ignored',
  });
...
test('custom paths take input, job environment and Linux default precedence', t => {
  const f = fixture(t);
  const config = { tools: 'cargo python bun trivy', 'python-manager': 'uv', 'cargo-target': 'true', 'cargo-cache-path': 'custom/cargo', 'uv-cache-path': 'custom/uv' };
  const p = f.plan(config, {}, { CARGO_HOME: '/unused', UV_CACHE_DIR: '/unused', BUN_INSTALL_CACHE_DIR: '/bun-cache', XDG_CACHE_HOME: '/xdg', CARGO_TARGET_DIR: 'out' });
  assert.equal(p.caches.cargo.path, ['registry/index', 'registry/cache', 'git/db'].map(x => path.join(f.root, 'custom/cargo', x)).join('\n'));
...
test('compiled cache separates target, variant and compiler files from download keys', t => {
  ...
  for (const variant of [{ 'cargo-build-variant': 'release-feature-a' }, { 'cargo-build-target': 'aarch64-unknown-linux-gnu' }]) {
...
test('PR cold fallback, success save, repeat reuse and default-branch exclusion', t => {
  ...
  const restore = cache => [cache.key, ...cache.restore.split('\n')].flatMap(prefix => [...entries.keys()].filter(key => key.startsWith(prefix)))[0];
...
test('concurrent identical dependency writers intentionally share an immutable logical key', t => {
  ...
  assert.notEqual(f.plan({ 'cargo-target': 'true', 'cargo-build-variant': 'one' }).caches['cargo-target'].key,
