...
function fixture(t) {
  ...
  for (const executable of ['bash', 'dirname']) {
    const found = spawnSync('bash', ['-c', 'command -v "$1"', 'lookup', executable], { encoding: 'utf8' }).stdout.trim();
  ...
  write('devenv.nix', '{}'); write('devenv.yaml', '{}'); write('devenv.lock', '{}');
  write('flake.nix', '{}'); write('flake.lock', '{}');
  const env = { ...process.env, PATH: bin, HOME: root, RUNNER_OS: 'Linux', RUNNER_ARCH: 'X64', RUNNER_ENVIRONMENT: 'self-hosted', GITHUB_WORKSPACE: root, PROJECT_DIRECTORY: root, GITHUB_OUTPUT: path.join(root, 'output'), GITHUB_PATH: path.join(root, 'paths'), TRACE: path.join(root, 'trace') };
  for (const key of ['buildInputs', 'nativeBuildInputs', 'propagatedBuildInputs', 'propagatedNativeBuildInputs']) delete env[key];
  return { root, bin, env, write, mock, run: (name, more = {}) => spawnSync('bash', [path.join(internal, name+'.sh')], { env: { ...env, ...more }, encoding: 'utf8' }) };
}

test('Nix is reused on both architectures and consecutive calls never install', t => {
  const f = fixture(t);
  f.mock('nix', 'printf "%s\\n" "$*" >>"$TRACE"; [[ $1 == --version ]]');
  for (const arch of ['X64', 'ARM64']) for (let i = 0; i < 2; i++) assert.equal(f.run('nix', { RUNNER_ARCH: arch }).status, 0);
...
test('shell dispatch preserves failures and noninteractive settings without losing profile', t => {
  ...
  const result = f.run('devenv', { ENVIRONMENT_TYPE: 'flakes', SECRETSPEC_PROVIDER: 'interactive', SECRETSPEC_ENV: 'custom', SECRETSPEC_REASON: 'test-reason' });
...
test('Trivy rejects ambient binary and reports declared package version', t => {
  ...
  for (const dispatcher of ['devenv', 'nix']) f.mock(dispatcher, 'if [[ -n ${PROJECT_BIN:-} ]]; then export PATH="$PROJECT_BIN:$PATH"; fi; while [[ $1 != bash ]]; do shift; done; exec "$@"');
  ...
  for (const type of ['devenv', 'flakes']) assert.equal(f.run('trivy', { ENVIRONMENT_TYPE: type, PROJECT_BIN: path.join(f.root, 'package/bin') }).status, 0);
