...
async function main() {
  ...
  try {
    ...
    const env = {
      ...
      GITHUB_REPOSITORY: "fixture/actions",
      GITHUB_WORKSPACE: root,
      RUNNER_TEMP: root,
      GITHUB_OUTPUT: path.join(root, "output"),
      GITHUB_STATE: path.join(root, "state"),
      INPUT_PATH: cache,
      INPUT_KEY: "fixture-test-key",
      "INPUT_RESTORE-KEYS": "fixture-",
      INPUT_ENABLECROSSOSARCHIVE: "false",
      "INPUT_LOOKUP-ONLY": "false",
      "INPUT_FAIL-ON-CACHE-MISS": "false",
      RUNS_ON_S3_BUCKET_CACHE: "fixture",
      RUNS_ON_S3_BUCKET_ENDPOINT: `http://127.0.0.1:${server.address().port}`,
