...
`,
    ...
    for (const mode of ["read", "write", "fork"]) {
      ...
      const selected = policy(
        { "cachix-name": "public-fixture", "cachix-token": token },
        {
          os: "Linux",
          arch: "X64",
          runner: "self-hosted",
          repository: "fixture/project",
          headRepository: mode === "fork" ? "fork/project" : "",
        },
      );
      const env = {
        ...process.env,
        ...
        TRACE: path.join(directory, "trace"),
        FIXTURE_USER: os.userInfo().username,
        INPUT_NAME: "public-fixture",
        INPUT_AUTHTOKEN: selected.cachix === "write" ? token : "",
        INPUT_SKIPPUSH: String(selected.cachix !== "write"),
        INPUT_USEDAEMON: "true",
        INPUT_SKIPADDINGSUBSTITUTER: "false",
        INPUT_CACHIXBIN: path.join(bin, "cachix"),
        INPUT_SIGNINGKEY: "",
      ...
      console.log(
        `Pinned Cachix ${mode}: substituter setup and post-job ${selected.cachix === "write" ? "daemon flush" : "no-push"} verified with mocks.`,
      );
    }
  } finally {
